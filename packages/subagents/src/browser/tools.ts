...
const selectorSchema = z.object({
  selector: z.string().optional().describe("CSS selector (optional, defaults to entire page)"),
});
  ...
  return {
    navigate: tool({
      ...
      execute: async ({ url }: z.infer<typeof navigateSchema>) => {
        const result = await daemonController.executeCommand<NavigateData>(sessionId, {
          id: createCommandId(),
          action: "navigate",
          url,
        });
        ...
        return { success: true, url: result.data?.url, title: result.data?.title };
      },
    }),
    ...
    type: tool({
      ...
      execute: async ({ selector, text, clear }: z.infer<typeof typeSchema>) => {
        const result = await daemonController.executeCommand(sessionId, {
          id: createCommandId(),
          action: "type",
          selector,
        ...
        logStep(
          "type",
          { selector, text: text.slice(0, 50) + (text.length > 50 ? "..." : "") },
          undefined,
          result.error,
        );
        if (!result.success) return { success: false, error: result.error };
    ...
    fill: tool({
      ...
      execute: async ({ selector, value }: z.infer<typeof fillSchema>) => {
        ...
        logStep(
          "fill",
          { selector, value: value.slice(0, 50) + (value.length > 50 ? "..." : "") },
          undefined,
          result.error,
        );
        if (!result.success) return { success: false, error: result.error };
    ...
    screenshot: tool({
      ...
      execute: async () => {
        ...
        logStep("screenshot", undefined, result.success ? "captured" : undefined, result.error);
        if (!result.success) return { success: false, error: result.error };
    ...
    getContent: tool({
      ...
      execute: async ({ selector }: z.infer<typeof selectorSchema>) => {
        const result = await daemonController.executeCommand<ContentData>(sessionId, {
          id: createCommandId(),
          action: "content",
          selector,
        });
        logStep("getContent", { selector }, result.success ? "retrieved" : undefined, result.error);
        if (!result.success) return { success: false, error: result.error };
    ...
    getSnapshot: tool({
      ...
      inputSchema: emptySchema,
      execute: async () => {
        const result = await daemonController.executeCommand<SnapshotData>(sessionId, {
          id: createCommandId(),
          action: "snapshot",
        });
        logStep("getSnapshot", undefined, result.success ? "retrieved" : undefined, result.error);
        if (!result.success) return { success: false, error: result.error };
        const snapshot = result.data?.snapshot ?? "";
        const truncated =
          snapshot.length > 10000 ? snapshot.slice(0, 10000) + "\n... (truncated)" : snapshot;
    ...
    getText: tool({
      ...
      execute: async ({ selector }: z.infer<typeof requiredSelectorSchema>) => {
        const result = await daemonController.executeCommand<{ text: string }>(sessionId, {
          id: createCommandId(),
          action: "innertext",
          selector,
        });
        logStep("getText", { selector }, result.data?.text?.slice(0, 100), result.error);
        if (!result.success) return { success: false, error: result.error };
    ...
    getCurrentUrl: tool({
      ...
      inputSchema: emptySchema,
      execute: async () => {
        const result = await daemonController.executeCommand<{ url: string }>(sessionId, {
          id: createCommandId(),
          action: "url",
        });
    ...
    getPageTitle: tool({
      ...
      inputSchema: emptySchema,
      execute: async () => {
        const result = await daemonController.executeCommand<{ title: string }>(sessionId, {
          id: createCommandId(),
          action: "title",
        });
    ...
    startRecording: tool({
      ...
      execute: async ({ url, timeout }: { url?: string; timeout?: number }) => {
        ...
        logStep("startRecording", { url, timeout: clampedTimeout }, undefined, result.error);
        if (!result.success) return { success: false, error: result.error };
        return {
          success: true,
    ...
    stopRecording: tool({
      ...
      inputSchema: emptySchema,
      execute: async () => {
        const result = await daemonController.executeCommand<RecordingStopData>(sessionId, {
          id: createCommandId(),
